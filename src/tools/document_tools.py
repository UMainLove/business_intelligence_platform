...
"""
        ...
        content = template.format(
            ...
            financial_projections=business_data.get(
                "financial_projections", "Financial projections pending"
            ),
            risk_assessment=business_data.get("risk_assessment", "Risk assessment pending"),
            implementation_timeline=business_data.get(
                "implementation_timeline", "Timeline pending"
            ),
            funding_requirements=business_data.get(
                "funding_requirements", "Funding requirements pending"
            ),
...
"""
        ...
        content = template.format(
            ...
            profitability_analysis=financial_data.get(
                "profitability_analysis", "Profitability analysis pending"
            ),
            ...
            sensitivity_analysis=financial_data.get(
                "sensitivity_analysis", "Sensitivity analysis to be performed"
            ),
            funding_requirements=financial_data.get(
                "funding_requirements", "Funding requirements to be determined"
            ),
            generation_date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
...
"""
        ...
        content = template.format(
            ...
            mitigation_strategies=risk_data.get(
                "mitigation_strategies", "Risk mitigation strategies to be developed"
            ),
            monitoring_plan=risk_data.get(
                "monitoring_plan", "Risk monitoring plan to be established"
            ),
            generation_date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        )
        ...
        filename = f"risk_assessment_{
            risk_data.get('business_name', 'business').replace(' ', '_')
        }_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        file_path = self.output_dir / filename
...
"""
        ...
        content = template.format(
            ...
            business_overview=session_data.get(
                "business_overview", "Business overview from analysis"
            ),
            market_opportunity=session_data.get(
                "market_opportunity", "Market opportunity identified"
            ),
            competitive_advantage=session_data.get(
                "competitive_advantage", "Competitive advantages noted"
            ),
            financial_highlights=session_data.get(
                "financial_highlights", "Financial projections summarized"
            ),
            key_risks=session_data.get("key_risks", "Primary risks and mitigation strategies"),
            funding_requirements=session_data.get(
                "funding_requirements", "Funding needs assessment"
            ),
    ...
    return {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "data": {"type": "object", "description": "Data to include in the document"},
            },
