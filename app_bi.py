...
from src.util import estimate_tokens_chars, estimate_cost_usd, get_cost_breakdown
from src.business_intelligence import (
    build_bi_group, run_sequential_validation, run_swarm_analysis,
    run_enhanced_synthesis, get_bi_capabilities
)
from src.workflows.swarm_scenarios import ScenarioType
from src.health_monitor import health_monitor
from src.error_handling import error_tracker, safe_execute
...
st.caption("AI-Powered Business Analysis with Market Research, Financial Modeling & Scenario Planning")
...
tab1, tab2, tab3 = st.tabs(["🎯 Interactive Analysis", "📋 Sequential Validation", "🌪️ Scenario Analysis"])
    ...
    if approx_tokens > 0:
        ...
        with st.expander("Enhanced BI Cost Breakdown", expanded=False):
            ...
            if cost_breakdown.get('cost_increase_vs_standard', 0) > 0:
                st.caption(f"**Cost increase vs standard:** +{cost_breakdown['cost_increase_vs_standard']:.1f}% (due to tool usage)")
            
            st.caption("**Token Distribution:**")
            breakdown = cost_breakdown['breakdown']
            st.caption(f"• Input (context): {breakdown['input']['percentage']:.1f}% | {breakdown['input']['tokens']:,} tokens | ${breakdown['input']['cost']:.4f}")
            st.caption(f"• Specialists output: {breakdown['specialists_output']['percentage']:.1f}% | {breakdown['specialists_output']['tokens']:,} tokens | ${breakdown['specialists_output']['cost']:.4f}")
            st.caption(f"• Synthesizer output: {breakdown['synthesizer_output']['percentage']:.1f}% | {breakdown['synthesizer_output']['tokens']:,} tokens | ${breakdown['synthesizer_output']['cost']:.4f}")
            if breakdown.get('tool_overhead'):
                st.caption(f"• Tool overhead: {breakdown['tool_overhead']['percentage']:.1f}% | {breakdown['tool_overhead']['tokens']:,} tokens | ${breakdown['tool_overhead']['cost']:.4f}")
            
            st.caption(f"**Model Rates:** Specialists ${settings.price_in_specialists}/1K in, ${settings.price_out_specialists}/1K out | Synthesizer ${settings.price_in_synth}/1K in, ${settings.price_out_synth}/1K out")
...
with tab1:
    ...
    # Chat input
    prompt = st.chat_input("Describe your business idea… (type DONE for comprehensive report, SEQUENTIAL for phased analysis, SWARM for scenario planning)")
    ...
    if prompt:
        ...
        if prompt.strip().upper() == "SEQUENTIAL":
            ...
            with st.spinner("Generating comprehensive analysis and documents..."):
                ...
                # Display synthesis
                st.session_state.history.append(("synthesizer", synthesis_result['synthesis_response']))
            ...
            with st.spinner("AI specialists analyzing with business intelligence tools..."):
                try:
                    ...
                    if chat_result is None:
                        st.error("⚠️ Chat interaction failed. Please try again with a different prompt.")
                        # Add error message to history
                        st.session_state.history.append(("error", "Chat interaction failed. The system may be experiencing issues."))
                    else:
    ...
    with st.form("sequential_validation"):
        ...
        with col1:
            ...
            industry = st.selectbox("Industry", [
                ...
                "AI/ML", "Marketplace", "Consumer", "Enterprise", "Other"
            ])
        
        with col2:
            target_market = st.text_input("Target Market")
            business_model = st.selectbox("Business Model", [
                "Subscription", "Marketplace", "E-commerce", "Advertising", 
                "Freemium", "Enterprise License", "Transaction Fee", "Other"
            ])
            region = st.selectbox("Primary Region", [
                "North America", "Europe", "Asia", "Global", "Other"
            ])
        ...
        if submitted and business_idea:
            ...
            for phase, result in validation_results.items():
                with st.expander(f"Phase {list(validation_results.keys()).index(phase) + 1}: {phase.value.replace('_', ' ').title()}", 
...
with tab3:
    ...
    st.markdown("Multi-agent scenario planning to stress-test your business under different conditions.")
    ...
    with st.form("swarm_analysis"):
        ...
        if run_swarm and business_idea_swarm:
            ...
            for scenario_type, result in swarm_results.get('scenario_results', {}).items():
                with st.expander(f"{scenario_type.value.title()} Scenario (Impact: {result.impact_score:.1f}/10, Probability: {result.probability:.1%})"):
