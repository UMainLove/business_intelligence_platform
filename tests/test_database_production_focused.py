...
class TestProductionBusinessDataDB:
    ...
    def test_query_industry_success_rates_sqlite(self, mock_sqlite_db_class, mock_db_config):
        ...
        mock_db_config.use_postgres = False
        mock_sqlite_db = Mock()
        mock_sqlite_db.query_industry_success_rates.return_value = {"industry": "SaaS", "success_rate": 75.0}
    ...
    def test_analyze_similar_ventures_sqlite(self, mock_sqlite_db_class, mock_db_config):
        ...
        mock_sqlite_db.analyze_similar_ventures.assert_called_once_with("SaaS", "subscription", "US")
...
class TestDatabaseToolExecutor:
    ...
    def test_success_rates_executor(self, mock_db_class):
        ...
        mock_db = Mock()
        mock_db.query_industry_success_rates.return_value = {"industry": "SaaS", "success_rate": 80.0}
...
class TestErrorHandling:
    ...
    def test_query_industry_success_rates_sqlite_error(self, mock_safe_execute, mock_sqlite_db_class, mock_db_config):
...
class TestIntegration:
    ...
    def test_full_workflow_postgres(self, mock_db_config):
        ...
        def mock_execute(query, params=()):
            if "SELECT COUNT(*)" in query:
                ...
                mock_cursor.fetchall.return_value = [
                    {"status": "active", "count": 2, "avg_funding": 3000000.0, "avg_valuation": 15000000.0}
                ]
            elif "SELECT metric_name" in query:
                mock_cursor.fetchall.return_value = [
                    {"metric_name": "CAC", "metric_value": 120.0, "metric_unit": "USD", "percentile": 50, "source": "Report"}
                ]
            elif "SELECT name, status" in query:
                mock_cursor.fetchall.return_value = [
                    {"name": "Similar1", "status": "active", "initial_funding": 1000000.0, 
                     "total_funding": 5000000.0, "valuation": 25000000.0, "employees": 50, "revenue": 3000000.0}
                ]
    ...
    def test_full_workflow_sqlite(self, mock_sqlite_db_class, mock_db_config):
        ...
        mock_sqlite_db.query_industry_success_rates.return_value = {"industry": "SaaS", "success_rate": 85.0}
        mock_sqlite_db.get_industry_benchmarks.return_value = {"industry": "SaaS", "metrics": []}
